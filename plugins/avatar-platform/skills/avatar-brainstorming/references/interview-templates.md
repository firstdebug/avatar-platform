...
**主题 0: 已有资源的使用确认**
```
只在用户提供 appId、sceneId、模型、知识库或 SDK 资产时提问：
"检测到你提供了 appId/sceneId 等已有资源。是否复用它们？我会先在线验证；sceneId 无效或未发布时，将在该 appId 下自动创建并发布新的接口场景。"

记录：
- resource_reuse: true | false
- invalid_scene_action: create_replacement | stop
```

在未取得答案前，不能把给出的 sceneId 写入设计或配置；但不得重复询问用户已经明确为“复用且自动创建”的偏好。
